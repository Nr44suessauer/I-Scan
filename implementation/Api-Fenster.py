...
class DeviceControl:
    ...
    def home_func(self):
        ...
        This function moves the stepper motor down until the button is pressed,
        then moves up slightly and resets the position to zero.
        ...
        try:
            ...
            # Reset button state - wait until button is no longer pressed
            self.logger.log("Resetting button state before starting home function...")
            reset_attempts = 0
            max_reset_attempts = 10
            
            # Loop waiting for button to be released
            while reset_attempts < max_reset_attempts:
                reset_attempts += 1
                btn_response = ApiClient.get_button_state(base_url, nocache=True)
                button_still_pressed = ApiClient.is_button_pressed(btn_response)
            ...
            # Reset complete, continue with home function
            ...
            while not button_pressed and attempt < max_attempts:
                ...
                self.logger.log(f"Button: Pressed: {button_pressed} (Attempt {attempt})")
                
                if button_pressed:
                    # Button pressed, 100 steps up and exit
                    self.logger.log(f"Button pressed detected on attempt {attempt}, moving up and completing home")
                    params = {"steps": 100, "direction": 1, "speed": speed}
                    ApiClient.make_request("setMotor", params, base_url)
                    pos_cm = self.position.get() + (100 / 4096 * PI * d / 10)
                    distance_cm = (100 / 4096 * PI * d / 10)
                    ...
                    # Explicit delay after each step
                    time.sleep(0.5)
            
            # If button was never pressed, issue a warning
            if not button_pressed:
