#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
ANGLE PROGRESSION VISUALIZATION MODULE
=====================================

Creates angle progression visualization showing how servo angles change
across measurement points.

Author: I-Scan Team
Version: 2.0 (Modular split from complete_servo_angle_explanation.py)
"""

import matplotlib.pyplot as plt
import os
from config import OUTPUT_DIR, ensure_output_dir


def create_angle_progression_visualization(angles_data):
    """
    Creates angle progression visualization
    """
    fig = plt.figure(figsize=(14, 10))
    fig.patch.set_facecolor('white')
    fig.suptitle('SERVO ANGLE PROGRESSION ACROSS MEASUREMENT POINTS', 
                 fontsize=16, fontweight='bold', y=0.95, color='navy')
    
    ax = plt.subplot(1, 1, 1)
    ax.set_title('Theoretical vs Corrected Servo Angles', 
                 fontsize=12, fontweight='bold', pad=20, color='darkblue')
    
    # Extract data
    y_positions = [data['y_pos'] for data in angles_data]
    theoretical_angles = [data['theoretical'] for data in angles_data]
    final_angles = [data['final'] for data in angles_data]
    
    # Plot lines
    ax.plot(y_positions, theoretical_angles, 'o-', color='#2E86AB', 
            linewidth=3, markersize=10, markeredgewidth=2, 
            markeredgecolor='white', label='Theoretical Angles')
    ax.plot(y_positions, final_angles, 's-', color='#A23B72', 
            linewidth=3, markersize=10, markeredgewidth=2, 
            markeredgecolor='white', label='Corrected Angles')
    
    ax.set_xlabel('Y-Position (cm)', fontsize=12, fontweight='bold')
    ax.set_ylabel('Servo Angle (°)', fontsize=12, fontweight='bold')
    ax.grid(True, alpha=0.4, linestyle='--')
    ax.legend(fontsize=12, frameon=True, fancybox=True, shadow=True)
    ax.set_facecolor('#f8f9fa')
    
    # Annotations
    for i, (y_pos, theo, final) in enumerate(zip(y_positions, theoretical_angles, final_angles)):
        ax.annotate(f'{theo:.1f}°', (y_pos, theo), textcoords="offset points", 
                    xytext=(0,10), ha='center', fontsize=8, fontweight='bold')
        ax.annotate(f'{final:.1f}°', (y_pos, final), textcoords="offset points",             xytext=(0,-15), ha='center', fontsize=8, fontweight='bold')
    
    plt.tight_layout()
    ensure_output_dir()
    output_path = os.path.join(OUTPUT_DIR, '02_angle_progression.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight', 
                facecolor='white', edgecolor='none', pad_inches=0.2)
    print(f"📊 Angle progression visualization saved: {output_path}")
    plt.close()
