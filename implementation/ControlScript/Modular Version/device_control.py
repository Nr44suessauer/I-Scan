...
class DeviceControl:
    ...
    def servo_auto_position_cmd(self):
        ...
        try:
            ...
            self.logger.log(f"Servo Auto-Position: Y={current_y:.2f}cm → Winkel={calculated_angle}°")
            self.logger.log(f"  Zielzentrum: ({angle_info['target_center'][0]}, {angle_info['target_center'][1]})")
            self.logger.log(f"  Entfernung zum Ziel: {angle_info['distance_to_target']:.2f}cm")
            self.logger.log(f"  Berechneter Zielwinkel: {angle_info['angle_to_target_deg']:.1f}°")
            
        except Exception as e:
            self.logger.log(f"Fehler bei Auto-Positionierung: {e}")
    
    def update_servo_target_center(self, center_x, center_y):
        """
        ...
        """
        self.servo_calculator.update_target_center(center_x, center_y)
        self.logger.log(f"Servo-Zielzentrum aktualisiert: ({center_x}, {center_y})")
